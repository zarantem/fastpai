...
async def create_new_user(body: UserCreate, db) -> ShowUser:
...
async def get_user_by_user_id(user_id: UUID, db: AsyncSession = Depends(get_db)) -> ShowUser:
...
async def update_user_by_id(user_id: UUID, body: UpdateUserRequest, db: AsyncSession = Depends(get_db)) -> UpdateUserResponse:
    ...
    if user is None:
        raise HTTPException(status_code=404, detail=f"user with id {user_id} not found")
    update_user_id = await _update_user(body=body, db=db)
